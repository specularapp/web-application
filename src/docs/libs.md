...
| Avatar sem foto | @dicebear/core e @dicebear/adventurer | rosto ilustrado determinístico pela semente, no estilo Adventurer (pedido de 2026-09-05; Lorelei ficou em uso de 2026-09-04 a 2026-09-05, trocado a pedido pelo traço mais expressivo, com cabelo, óculos e brincos sorteados pela semente). Os dois estilos têm o mesmo único `id` e o mesmo `<metadata>`, então a troca foi só o pacote: SVG gerado em código, sem DOM, então sai igual no servidor e no cliente, sem folha injetada nem nonce. **Em uso** pelo `Avatar`, só quando não há `src`; entra por `components/ui/avatar/shape.ts` e nunca direto em feature, e desde 2026-09-08 o desenho é feito na rota `/api/avatar/[token]`, não mais embutido na página. O único `id` do SVG é reescrito com o hash da semente (o `randomizeIds` da lib usa Math.random e discordaria na hidratação) e o `<metadata>` sai. Só o pacote do estilo, e não a `@dicebear/collection` inteira | avvvatars-react (em uso de 2026-08-29 a 2026-09-04: formas abstratas em vez de rosto, e trazia o goober, que injetava `<style>` e exigia `window.__nonce__`), boring-avatars (menos variedade) |
