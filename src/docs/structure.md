...
- A frase e o botão do convite de plano são sorteados a cada carga, no servidor, entre os convites de `features/billing/pitches.ts`: a mesma frase todo dia vira papel de parede e a pessoa para de ler. Cada convite aponta para um recurso real do catálogo do Pro, nunca para promessa vaga; o título continua dizendo o estado (teste em andamento ou gratuito).
...
- Com o menu aberto a barra encolhe até sobrar só o X e vira bolinha; ao fechar, expande de volta. O grupo de busca e sino recolhe pela trilha da grade, de `1fr` a `0fr`, que o navegador anima como número, porque largura `auto` não anima e trilha anima; o filho tem largura mínima zero e recorta. Vão e canto transitam junto, senão ficava um vazio entre o grupo recolhido e o X. O botão remonta a cada troca e o glifo que chega gira ao entrar, porque X e três linhas não se transformam um no outro.
