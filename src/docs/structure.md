...
- Mobile: a imagem é fundo fixo da tela e o logo fica sozinho no topo, em branco (`--color-brand` trocado por `--color-on-accent` só nesse breakpoint) sobre um degradê fixo de `--color-scrim` que desce do topo até 32%, para o logo ler em qualquer imagem. A fusão com o fundo é ancorada no bloco do formulário, não na tela: o bloco tem fundo sólido `--color-bg` e um pseudo-elemento de 40dvh logo acima dele faz o degradê de `transparent` até `--color-bg`, então a transição termina sempre antes do primeiro rótulo, em qualquer altura de aparelho e nas duas etapas. Sem caixa, sem divisor, título só para leitor de tela, respiros de 16.
