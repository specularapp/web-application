...
- E-mails de autenticação com a nossa marca: templates em `db/supabase/templates/` (confirmação, convite, magic link, redefinição, troca de e-mail e reautenticação), HTML de e-mail com estilos inline e tabela, logo em PNG (gerado do SVG, e-mail não renderiza SVG) e botão preto. `config.toml` aponta os arquivos para o ambiente local; no hospedado é colar no painel e ligar o SMTP do Resend (passo a passo em `setup.md`). Cores literais aqui são exceção consciente: e-mail não tem tema nem tokens.
- A tela de confirmação do cadastro tem "Reenviar e-mail" (`resendConfirmation`, com rate limit e trava de 30s no botão), e o `/login` lê o fragmento `#error` que o Supabase devolve quando o link falha e explica em português (link expirado ou usado), limpando o hash da URL.
