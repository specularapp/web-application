...
- Rótulo de rota em contraste cheio (`--color-label`), e não secundário: a 60% ele sumia no tema claro. Quem carrega a hierarquia é o ícone, um degrau abaixo, e o item em vigor, que ganha peso médio e preenchimento. Os três botões do topo usam a variante `secondary`, ou seja, nascem com o fundo que o `ghost` só mostra no hover, para lerem como ação e não como enfeite.
