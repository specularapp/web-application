...
`auth`, `organizations`, `onboarding`, `dashboard`, `crm`, `clients`, `quotes`, `contracts`, `billing`, `projects`, `portfolio`, `resume`, `finance`, `automations`, `ai`, `gamification`, `settings`.
...
- `Avatar` recebe `name` e, opcionalmente, `src` e `seed`. Sem imagem entra o rosto do DiceBear no estilo Lorelei (`@dicebear/core` e `@dicebear/lorelei`, por `avatar/shape.tsx`, o único ponto que importa a lib), desenhado a partir do `seed`, com o e-mail como semente para o avatar não mudar quando o nome muda. Trocou o `avvvatars-react` em 2026-09-04, a pedido: rosto no lugar de forma abstrata. Iniciais continuam descartadas, por decisão de produto. O SVG é gerado em código, sem DOM, então sai igual no servidor e no cliente, sem folha injetada nem nonce (o `GooberNonce` saiu junto). O único `id` do SVG é reescrito com o hash da semente, porque a mesma máscara em vários avatares colidiria e o `randomizeIds` da lib usa Math.random, que discordaria na hidratação; o `<metadata>` sai para não pesar em cada avatar.
- O fundo do rosto é um matiz da paleta do sistema escolhido pela mesma semente (`hashString` de `lib/utils/hash.ts`, o mesmo da frase do dia), em pastel a 32% sobre o branco fixo (`--color-on-accent`), igual nos dois temas: o traço do Lorelei é escuro e sumiria sobre fundo escuro, e o pastel opaco é o que deixa um avatar cobrir o vizinho no grupo. O raio e o recorte continuam no invólucro, que já esconde o overflow por causa da foto, então círculo e squircle saem de um lugar só.
...
- O cartão do rodapé é um mural de sugestões do sistema, e não só convite de venda: ação, função, atalho, segurança ou plano, cada uma com ícone, categoria, chamada, frase e destino próprios, em `components/layout/suggestions.ts`, ao lado de `nav.ts` e com `href` tipado por rota do mesmo jeito. O servidor sorteia uma por carga e passa só o id. O módulo é lido pelo `AppShell`, que é Server Component, então o ícone vive nele como nome, e não como componente: o Phosphor cria contexto React ao carregar e, no servidor, isso derruba o build inteiro (`createContext is not a function`). O menu, no cliente, resolve o nome no componente por `suggestionIcons`. Sugestão de plano só entra no sorteio enquanto há o que vender. O botão do cartão navega de verdade para o destino.
...
- `PageHeader` (`components/layout/page-header/`) é o cabeçalho genérico de página: `leading`, título, linha de apoio e `actions`, Server Component com CSS Module. Título e linha de apoio ficam a 2px um do outro, porque são uma identidade só. Em tela estreita as ações descem para a linha de baixo em vez de sumir, salvo em `compact`: aí no celular sobra só o `leading` na tela, o texto vai para o leitor de tela e as ações ficam na mesma linha da foto. O painel usa `compact`, e o botão de criar orçamento vira só ícone no celular por CSS (os dois existem na marcação, cada largura mostra um), para não saltar na hidratação.
...
- Ações só em ícone, nesta ordem: buscar (abre a mesma busca do menu), período e a engrenagem de personalizar o painel, que vai escolher quais blocos aparecem e ainda não faz nada. Criar orçamento é a ação principal, a única com texto, e leva a `/orcamentos/novo`.
- O período é um `Listbox` em `iconOnly`, modo novo do componente: gatilho quadrado como o `IconButton`, só com o ícone de calendário (prop `icon`), e o valor em vigor no nome acessível e marcado na lista. As variáveis locais do gatilho o vestem de botão fantasma `sm`, para a fila de ícones ler como uma só. A escolha vai para `?periodo=` por `router.replace`, validada com zod no servidor (`features/dashboard/period.ts`; valor inválido cai em "Este mês"), porque é ela que os blocos do painel vão ler. Na vitrine o `demo` guarda a escolha só na tela.
- `DashboardScreen` (`features/dashboard/components/`) é a tela inteira, cabeçalho e grade, montada pela página com dados do banco e pela prévia `/previa/painel` com os dados de exemplo do menu, dentro da `AppFrame`, que é a moldura do `AppShell` sem o banco. A tela traz o próprio recuo, 16 no celular e 24 daí para cima, sem teto de largura: o painel aproveita a tela inteira. A concha continua sem recuo.
- A grade (`dashboard-grid.module.css`) tem sete blocos por áreas nomeadas, e o id de cada bloco em `features/dashboard/blocks.ts` é a área: mover um bloco é mexer só no mapa. Todo bloco tem duas linhas de altura, menos projetos e conquistas, que dividem a coluna que os outros ocupam inteira. Uma coluna até 48rem; duas até 72rem, em três pares de blocos altos; três colunas a partir de 72rem (896px de conteúdo ao lado do menu, a largura da referência), no mapa pedido: projetos e conquistas na primeira coluna, financeiro e clientes ao lado, e tarefas, equipe e último orçamento embaixo, todos altos. No desktop a altura é pré-definida, e não do conteúdo (decisão de 2026-09-04): as quatro linhas dividem o que sobra abaixo do cabeçalho entre um piso de 11rem e um teto de 60rem na grade, e o piso é fixo em vez de `auto`, então bloco nenhum estica a linha e gera rolagem. Só em tela mais baixa que o piso a página rola. No celular e no tablet a página é alta por natureza, e a linha tem só o piso de 8rem e cresce com o conteúdo. Vão de 16 entre blocos e 24 entre cabeçalho e grade. Cada bloco leva o ícone da rota correspondente no menu e um atalho tipado por rota para a tela dela; tarefas, sem tela, fica sem atalho.
- `Card` (`components/ui/card/`) é a caixa de cada bloco, sem fundo: quem desenha a caixa é o fio do menu (0,6px em `--color-border`), e o cabeçalho, com ícone na cor do título, título e a ação na outra ponta, se separa do corpo por outro fio de borda a borda. A ação é atalho e não foco: `Button` `outline`, `sm` e raio `md`. Raio `xl` com recuo 16 no cabeçalho e no corpo, então o que for aninhado cai no raio 8, exposto em `--card-inner`. O cabeçalho tem a altura do botão `sm` mais o respiro mesmo sem ação, para os títulos dos vizinhos ficarem na mesma linha. Server Component com CSS Module, canto por `squircle("xl")` sem recorte porque tem borda.
- Bloco de projetos (`ProjectsBlock`, 2026-09-04): à esquerda o total ("12 Projetos" em `title3`) com a linha de apoio "Todos os projetos da sua equipe" e, colado na base, as bolinhas de 36px dos três primeiros clientes (no celular entram 12px uma sobre a outra em vez de 8, pela variável `--avatar-overlap` do `AvatarGroup`, e o total encurta por reticências se ainda faltar) com o total de clientes em cor secundária, para pesar menos que o número de cima; à direita o gráfico dos últimos meses. O bloco é duas metades iguais, sempre lado a lado, e nenhuma empurra a outra (pedido de 2026-09-04): quem se adapta é o gráfico, que mede a própria caixa com `ResizeObserver` e mostra só os meses que cabem (32px por mês, no mínimo três, no máximo os que o resumo trouxer), cortando os mais antigos. Sem medida, no servidor e no primeiro quadro, ele não desenha, para nascer no tamanho certo em vez de pular. O gráfico é Recharts com `responsive`, sem eixo, grade ou texto nenhum: duas barras por mês, projetos iniciados no roxo do sistema a 35% e entregues no roxo cheio, porque são a mesma medida em dois tempos e a regra do tema é opacidade para sequência. Roxo, e não o acento azul, por decisão de produto do mesmo dia. As barras andam juntas (`barGap` 2 e `barCategoryGap` 12%), com o topo arredondado. O número de cada mês aparece só ao passar o ponteiro: uma faixa no preenchimento de hover da casa marca o par apontado, e a dica é de vidro, na receita das camadas da casa (fundo a 20% com borrão, fio fino, sombra e fade curto de entrada), com o mês apagado em cima e uma linha por série, bolinha na cor da barra, nome e o número na outra ponta, posicionada pelo Recharts e presa dentro da área do gráfico (sem `allowEscapeViewBox`), porque solta ela saía da tela no celular, com o nome do mês saindo da chave `yyyy-MM` no cliente, com `date-fns` em pt-BR. O SVG fica fora da árvore acessível e a leitura por voz é uma lista oculta ao lado. Em bloco estreito o gráfico desce para baixo do resumo. O resumo chega por prop (`ProjectsSummary`, em `features/projects/summary.ts`) e hoje vem de `features/projects/preview.ts`: **o domínio não existe no banco**, e quando existir muda só a origem.
