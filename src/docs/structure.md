...
- `Avatar` recebe `name` e, opcionalmente, `src` e `seed`. Sem imagem entra o `avvvatars-react` (por `avatar/shape.tsx`, o único ponto que importa a lib), que gera cor e iniciais determinísticas a partir do `seed`, com o e-mail como semente para a cor não mudar quando o nome muda. O raio e o recorte continuam no invólucro, que já esconde o overflow por causa da foto, então círculo e squircle saem de um lugar só e a lib entrega só cor e letra.
