...
- Configuração inicial do time em `/(app)/primeiros-passos`, logo depois do cadastro e da MFA. Não é tela de autenticação: tema normal (claro ou escuro conforme a preferência) e nada de cabeçalho com logo e sair.
- A tela é uma camada sobre a aplicação, não uma página com fundo próprio: `position: fixed` cobrindo tudo, véu em `color-mix(--color-bg-grouped 72%)` e `backdrop-filter`, com o painel de vidro por cima (fundo em 76%, borda, sombra e blur). O véu segue o tema em vez de usar `--color-scrim` preto, senão o tema claro escureceria inteiro; e como o desfoque pega o que estiver atrás, o painel já nasce pronto para o dashboard com conteúdo. Alinhamento é `align-content: safe center`, que centraliza quando cabe e vira topo quando o conteúdo passa da tela, sem cortar a primeira linha.
- As três barras de etapa ficam **fora** do painel, logo acima, sem rótulo de texto. A troca é estado do cliente com `key` por etapa, então cada bloco monta de novo e roda `stepEnter` (opacidade e 8px de deslize), no mesmo espírito do `authEnter`.
- Etapa 1: logo, nome, site e área de atuação. A logo é só o alvo de envio por `react-dropzone` (PNG, JPG ou WEBP até 2 MB), quadrado centralizado com ícone de nuvem e sem texto ao lado; a imagem escolhida ocupa o quadrado inteiro.
...
- "Site" é o endereço do próprio cliente, opcional, com `https://` como `FieldAffix` fixo, o mesmo tratamento do `R$` e do `%`: o campo guarda só o domínio, colar a URL inteira tira o protocolo repetido e `normalizeWebsite` recoloca antes do zod validar.
- As barras de etapa levam o mesmo recuo lateral do padding do painel, então começam alinhadas ao título de dentro dele.
- O alvo de envio da logo centraliza o ícone numa camada absoluta própria, e não pelo grid do alvo: o `input` de arquivo do `react-dropzone` é filho real e entraria na conta do grid, empurrando o ícone para baixo do centro.
