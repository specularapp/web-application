...
- Todo canto arredondado usa `border-radius` tradicional com os tokens `--radius-*`. Componentes em TypeScript usam os helpers `rounded`, `roundedAuto` e `roundedPx` de `src/lib/corners.ts` quando o raio não puder ficar só no CSS. Não usar `corner-shape`, superelipse, motor JavaScript ou recorte por caminho. Pílulas e círculos usam `--radius-full`. Contrato completo na seção Cantos de `src/docs/structure.md`.
