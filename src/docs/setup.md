...
3. Authentication > URL Configuration:
   ...
   - Redirect URLs: `https://app.specular.com.br/**` e `http://localhost:3000/**`. O curinga é obrigatório: o nosso `redirectTo` leva `?next=...`, e entrada sem curinga não casa com a query, então o Supabase ignora o retorno e joga a pessoa no Site URL, onde o código do OAuth se perde e a sessão nunca nasce.
...
5. O logo dos e-mails é `public/logotipo/specular-icon-email.png`: o ícone da marca dentro de um chip branco com borda, embutidos no próprio PNG. O motivo é o modo escuro dos clientes de e-mail (Gmail, Outlook), que invertem fundo e texto mas não mexem em imagens: um ícone preto solto sumia no fundo escurecido, o chip claro continua legível nos dois temas. Servido em `https://app.specular.com.br/logotipo/...`; publicar antes de testar o e-mail em produção.
