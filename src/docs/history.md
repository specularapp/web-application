...
Tempo: em andamento (madrugada, continuação do dia 14).

Feito:

- **O editor de contratos lapidado, a pedido** ("coisas importantes"): o cabeçalho dentro da folha saiu, na tela e no PDF (o título abre o documento, como na norma; quem emite foi para o rodapé do PDF); a barra de ferramentas ficou na largura do que tem dentro, centrada sobre a folha; **a coluna da direita saiu e tudo o que ela tinha entrou num leque só, "Opções"** (nova cláusula já numerada na forma da norma com o assunto selecionado para digitar por cima, lista, numerada e citação em interruptor, linha, link com a linha de endereço embaixo da barra, os dados do contrato no cursor e o estilo do documento com a bolinha da cor); a ficha da esquerda foi para 22rem em três grupos com rótulos curtos, porque os longos quebravam linha; e a folha ganhou a proporção A4, crescendo com o texto. A margem lateral da norma estava invertida no CSS (2 cm à esquerda e 3 à direita) e foi corrigida.
- **Tema escuro sobre a folha branca**: o vidro a 20% das camadas ficava ilegível por cima do papel, então o `DropdownMenu` ganhou a superfície `solid` (a pedido geral de "a página no tema preto está bugada"), o tipo do bloco virou leque para usar a mesma peça, a barra e a linha do link passaram para o vidro da bandeja, e o container flutuante das páginas públicas de contrato e de orçamento também.
...
- **O seletor de equipe ganhou ações por linha**, a pedido: o leque da casa em cada equipe, com Editar (a gaveta de criar agora edita, lendo a equipe pelo id) e **Arquivar**, que é o lugar do excluir por decisão do usuário. Migração `organizations.archived_at`, a listagem filtrando, `set_current_org` recusando equipe arquivada e `set_organization_archived` exigindo ser dono; Server Action e `PATCH` em `api/v1/organizacoes`. Migração aplicada no banco e tipos regerados; o `setClientFlag` precisou trocar a chave dinâmica por coluna por extenso, porque os tipos novos recusam chave montada em texto.
- **Quatro defeitos relatados, corrigidos na causa**: (1) a caixa colada no gatilho perseguia o gatilho a cada evento de rolagem, um render por evento, e andava aos trancos; agora ela **trava a rolagem de quem está atrás**, como a janela já fazia, e só a dica de hover segue sem travar. (2) Escolher mês ou ano fechava o calendário, porque o seletor abre por portal e o toque caía "fora": o calendário flutuante **entrou na fila de camadas**, então enquanto o seletor está aberto quem responde é ele. (3) "Salvar como rascunho" girava sem fim: a escrita ficava presa derrubando o cache, porque o Redis não tinha **teto por comando** nem fila de espera desligada; e, para o resto, toda chamada de action da tela passou a ter **teto de tempo** (`lib/action.ts`, 102 chamadas), que nunca deixa o botão girando e diz para conferir antes de repetir, porque o servidor pode ter concluído. (4) **Projeto sem cliente** agora existe: migração soltando o `not null`, junção à esquerda no lugar da interna (com a interna ele sumia da lista), "Sem cliente" no topo do seletor e o glifo de projeto independente no lugar do rosto no cartão e na ficha.
...
- **A página de contratos, de ponta a ponta**, a pedido e em blocos: primeiro o topo e a `PageToolbar`; depois, com a visão completa do usuário (três origens, e-mail às duas partes, assinatura registrada), **a listagem em cartões no estilo da referência** com filtros e paginação, **a janela de criar** com as três origens (o PDF sobe por rota em `multipart`, o modelo abre a galeria, o do zero nasce com o esqueleto), **o editor** em três colunas sobre a referência de editor de notas (ficha, folha com Tiptap, ferramentas com estilo, texto e reescrita por IA; no PDF, os campos arrastáveis sobre as páginas do pdf.js), **a janela do contrato** com o documento, as partes e a linha do tempo, **o envio por e-mail** com link pessoal por parte, **a página pública de assinar** com o quadro de assinar à mão, e **o PDF final** com as assinaturas (react-pdf no documento escrito, `pdf-lib` carimbando o anexado). Tudo sobre o store em memória, com a assinatura que a tabela vai herdar.
- **O documento na norma brasileira de contratos**, a pedido do mesmo dia ("um documento mais sério, com a nossa identidade e fontes"): título em caixa alta centrado, preâmbulo com as partes qualificadas, cláusulas em ordinal e caixa alta com dois pontos (sem travessão, pela regra da casa), itens numerados, fecho com local e data; mancha de 3 e 2 cm, corpo de 12 pontos, entrelinha 1,5, justificado, na tela e no PDF. Oito modelos refeitos nessa forma e o esqueleto do zero também.
- **Bibliotecas novas**, registradas em `libs.md`: `pdfjs-dist` para ver as páginas no navegador e `pdf-lib` para escrever no PDF existente; o Tiptap, já mapeado, entrou em uso com `extension-text-align` e `extensions`, presos na versão do `core` instalado. O registro da Inter para o react-pdf saiu do orçamento para `lib/pdf/fonts.ts`, e a resposta de PDF para `lib/pdf/response.ts`.
- **O editor refeito sobre as referências**, a pedido: a barra de ferramentas passou para cima da folha, como num processador de texto, com o alinhamento em leque e **a IA num único botão em leque** (ações sobre o trecho e a reescrita), que é o mesmo do menu que flutua sobre a seleção; as quatro sugestões soltas no painel saíram. O painel da direita ficou com o estilo em amostras com check e os blocos em azulejos com nome embaixo.
- **Acertos medidos no navegador**: a folha do editor em fluxo de bloco, porque em grade ou flex com altura mínima a linha do texto ficava com o que sobrava da altura e as assinaturas pousavam por cima do texto; a coluna do PDF com largura no palco, porque centrada na grade ela encolhia para 24px; e a sonda de captura passou a abrir a própria aba, porque o Chrome é compartilhado com outra sessão e a primeira aba da lista era a tela de outra pessoa.
- **O SpeculAI**, o assistente de IA da casa, batizado no mesmo dia, sobre referências do usuário (o Gemini dentro do Pinterest e dentro do Chrome, os compositores do Claude e do ChatGPT): a coluna entra na **grade da concha** e empurra a página, como o menu lateral faz do outro lado, em vez de cobrir o conteúdo com uma janela. A concha ganhou uma terceira trilha `auto`; a coluna anda na própria largura e a caixa de dentro desliza em `translate`, no mesmo tempo. Entre 48 e 64rem ela flutua sobre o conteúdo em vez de empurrar, porque empurrando sobrariam duzentos pixels de página; no celular é a bandeja da casa, com 78dvh e as ações do compositor na barra flutuante. Conferida no navegador nas três larguras com a sonda de tela.
- **O widget da IA no topo virou o gatilho** (a pedido) e trocou a marca da OpenAI pela da casa: ele abre e fecha a coluna, fica preenchido enquanto ela está aberta, e o resumo do uso do ciclo, que ele abria antes, virou atalho de dentro dela. A IA passou a morar num lugar só.
- **Anexo que fica, histórico e saldo** (a pedido, no fim do dia): o arquivo e o áudio saem do compositor ao enviar e passam a morar na pergunta, acima do balão; "nova conversa" guarda a de agora num histórico no cabeçalho, em menu da casa, e voltar a uma troca de lugar com a de agora; e o pé do compositor mostra quantas ações do ciclo ainda restam. O histórico vive na memória da sessão, porque conversa é conteúdo e a casa não guarda conteúdo em cookie.
- **A conversa, simulada ponta a ponta**: pergunta em balão à direita, resposta solta na coluna com a marca da casa na margem, os passos do pensamento aparecendo um a um antes da primeira palavra, o texto surgindo palavra por palavra saindo do borrão, os registros lidos em pílulas no fim, copiar e refazer, parar no meio e começar conversa nova. Quatro respostas escritas sobre os dados de exemplo da aplicação, escolhidas por palavra da pergunta. Quando a rota de IA existir, `ask` chama `api/v1` e o resto da tela não muda.
- **O vazio e o compositor do assistente.** Marca e saudação pelo primeiro nome no meio da coluna, sem mais nada: as pílulas de sugestão passaram por três desenhos e saíram todas a pedido, e o convite virou o próprio campo. O compositor segue o cartão do comentário da tarefa (o cartão é o campo), com o clipe abrindo Imagem ou Documento, o modo da resposta em Rápido ou Avançado, ditar e enviar, reaproveitando o gravador e o tocador da conversa da tarefa. A tela aberta atrás vai junto da pergunta, na primeira linha de dentro do cartão, com o glifo dela e o × que a dispensa até a próxima tela.
- **A cara do assistente**: a coluna é o vidro da janela de plano borrando três manchas de cor pintadas **atrás** dele (com elas por dentro o vidro não tinha o que borrar), mais um token novo para a cor da IA (`--color-ai`, azul calmo) e o degradê da marca (`--gradient-ai`, quatro paradas varrendo do azul ao amarelo, com halo, no espírito do Gemini sem ser a paleta dele). A marca virou uma peça só, o `AiMark`, e aparece igual no cabeçalho da coluna, no vazio, na margem de cada resposta e no topo de cada página. Quatro rodadas de acerto no mesmo dia, a pedido: cartão solto com canto virou faixa colada como o menu; terracota virou azul; a pílula de buscar na conta saiu, porque ler a conta é o que o assistente é; e o cabeçalho passou a medir pelo `--topbar-height`, que o `Topbar` agora também lê, para a emenda entre as colunas ser uma linha só.
- Domínio novo em `features/ai/`: `conversation.ts` (fala, blocos, palavras e passos), `models.ts` (os dois modos), `AiVoice`, `AiAttachment` e `firstName` no `summary.ts`, e as sugestões e respostas de exemplo no `preview.ts`. Uso, nome e sugestões chegam por prop pela `AppFrame`, como manda o padrão de prévia: o domínio segue **sem existir no banco**.

Pendências:

- Assistente: nada disso fala com modelo nenhum. Falta a rota (`features/ai/service.ts` mais Server Action e `api/v1`, como manda a regra do celular), o fluxo de resposta de verdade, o histórico de conversas, o "@" para trazer um registro pelo nome, e a contagem de uso descontando a cada pergunta.
- Assistente: o anexo escolhido aparece na fila mas não sobe para lugar nenhum, como em toda a aplicação enquanto o armazenamento de arquivo não existe.
- Contratos: o domínio no banco (tabelas, `service.ts`, `api/v1`, arquivo no Storage); dados das partes do cadastro em vez de colchetes; itálico no PDF.
## 2026-09-13
...
- **A página de projetos começou**, sobre uma referência de cartão de projeto do usuário, na estrutura da base de clientes e do catálogo, um bloco por vez: primeiro a moldura (o `Topbar` da casa e a trilha vazia), depois a `PageToolbar` com busca, filtros e "Novo projeto", e a grade de cartões com a paginação presa no pé. O domínio ganhou o modelo `Project` (ao lado do resumo que o bloco do painel já usava), os rótulos (`labels.ts`: situação, ferramentas, faixa de valor sem centavo e a entrega em palavras), o lado leve (`list-options.ts`), o zod da URL (`list.ts`), o cookie da grade e uma prévia de 27 projetos, com os nove que já existiam em outras telas (quadro de tarefas, fichas de cliente e de membro) no mesmo `slug` e no mesmo id.
- **O cartão** segue a referência adaptada aos padrões da casa: quem contratou com rosto, empresa e data de começo, a situação em etiqueta e o leque; a capa em 16 por 9 no raio concêntrico, com a arte gerada no matiz do projeto enquanto a pessoa não anexa a imagem (decisão do dia: a capa é a imagem anexada pelo usuário); o título em duas linhas; as etiquetas em contorno no canto da casa, e não em pílula; e os fatos do pé com glifo, a ferramenta com a marca em cores, a faixa de valor, a entrega e as mensagens por ler, só o que existe. Concluído e cancelado apagam o que é do projeto.
- **Os filtros**: situação com a contagem da base inteira, entrega em quatro janelas (o vencido conta em todas, como no prazo das tarefas), etiqueta da base inteira e o interruptor de mensagens novas, com Limpar filtros. Sem seletor de visão, porque só a grade existe. `typecheck`, `lint` e `build` passam.
- **Cinco acertos a pedido, no mesmo dia.** As mensagens novas saíram do cartão, do modelo e do filtro. As ferramentas viraram lista (podem passar de vinte) e entram na fila agrupada da casa, até cinco marcas em cores e o resto em "+N" que lista as demais ao apontar, pelo `Tooltip`. A capa ganhou o canto `md` pelo sistema de cantos, porque o concêntrico de 4px saía reto demais para a imagem anexada. A entrega passou para a etiqueta do prazo das tarefas, com o calendário e a data curta, no pé do cartão ao lado das ferramentas, na receita do cartão de tarefas. E o título virou o **nome do projeto**, o do site ou de para quem o trabalho foi feito ("Estúdio Aurora", "Bravo Studio"), com o **endereço** embaixo em link; o que foi feito fica nas etiquetas. A lista fechada de ferramentas cobre as 43 marcas que existem em `public/brands`.
- **A janela do projeto**, a pedido de uma página de visualização no modal central: a `Dialog` `xl` da casa com a ficha à esquerda (capa, identidade, atalhos para o site e para o quadro de tarefas, azulejos com a barra de andamento, e as seções de perfil: sobre, detalhes, ferramentas, etiquetas, tarefas, orçamentos, contratos e cobranças) e a equipe com a atividade recente à direita; endereço em `/projetos/<id>` por `pushState`, e a página `[id]` abrindo a mesma tela com a janela pronta. A ficha completa (`ProjectDetails`) é buscada ao abrir por `loadProjectAction` e montada das prévias de tarefas, orçamentos e equipe, para o projeto já nascer ligado ao resto da casa. Junto, três acertos no cartão a pedido: o endereço virou botão de ícone na linha do nome, a faixa de valor saiu do cartão e ficou na ficha, e projeto sem ferramenta mostra uma bolinha de interrogação no lugar da fila vazia. `typecheck`, `lint` e `build` passam.
- **A janela lapidada, a pedido**: a capa deixou de quebrar a ficha, os atalhos viraram glifo e entraram a descrição e o **público**. A quebra foi medida no navegador antes de mexer: a capa tinha `aspect-ratio` e a arte dentro em porcentagem, e como item de uma grade com altura definida e rolagem por dentro a linha da capa resolvia para **0px** enquanto a caixa pintava 366px, então a ficha inteira nascia por cima dela. Altura declarada em rem na capa e na arte, e a linha da grade passou a medir o que a capa tem. Os três atalhos (site, editar, quadro de tarefas) ficaram só em glifo, com o nome na dica e na voz; "Sobre" saiu, porque a descrição agora abre a ficha logo abaixo da identidade; Detalhes ganhou Portfólio; e a janela refaz a ficha completa quando o projeto muda, e não só quando o id muda, para salvar devolver a ficha já com os dados novos.
- **Criar e editar funcionando de ponta a ponta** (`ProjectFormDialog`, `schemas.ts`, `actions.ts`, `store.ts`): a gaveta lateral da casa com três blocos (o projeto, prazo e valor, ferramentas e etiquetas), zod no servidor devolvendo o campo com problema para o campo certo, e o projeto novo ou editado aparecendo na lista refeita pelo servidor. Cada estado tem endereço: `/projetos/novo`, `/projetos/<id>/editar` e `/projetos/<id>`, todos a mesma tela, com o que está por cima mandando no caminho. A capa é **redimensionada no navegador** para 1440 por 810 e vai embutida em WebP, porque o envio da action tem teto; o storage entra quando existir. Enquanto o domínio não está no banco, os projetos vivem num store em memória do servidor (`store.ts`, `server-only`), semeado pela prévia: é a ponte com a assinatura que a tabela vai herdar.
- **A bolinha da ferramenta refeita, arquivo por arquivo** (a pedido: "logos da Adobe com fundo preto, alguns SVG quebrados, bg do Webflow e do TypeScript quebrados"). A primeira versão pintava toda marca em máscara de uma cor só, e os arquivos de `public/brands` não são todos da mesma natureza: medi os 43 num canvas, pela cobertura do quadro, pelas quinas opacas e pelas cores, e eles se dividem em azulejo inteiro (o TypeScript e o Webflow viravam quadrado chapado), forma cheia com o desenho vazado (o Next.js virava círculo cheio) e glifo solto. Agora cada marca declara como entra (`tile`, `mask` ou `glyph` em `projectToolBrands`), a marca vai por imagem de fundo no CSS, toda bolinha tem fio por dentro e `overflow: hidden`, e o azulejo cobre o círculo em vez de passar por cima da vizinha. Conferido numa folha de contato das 43, sobre fundo claro e escuro, que pegou o Webflow sumindo no próprio azul (é vazado, então o fundo precisa ser branco).
- **Defeito de casa achado no caminho**: o `BrandIcon` em cores passava pelo otimizador de imagem do Next, que recusa SVG sem `dangerouslyAllowSVG` e devolvia **400**, então a marca em cores não aparecia em lugar nenhum do produto (OAuth, autenticador, anexo de tarefa, movimentação do caixa). Agora vai `unoptimized`, direto do arquivo; ligar a opção do Next serviria qualquer SVG de fora pelo nosso domínio, e as marcas são arquivos nossos.
- **As etiquetas do projeto viraram gama pronta** (`tags.ts`), a pedido, no mesmo contrato das tarefas: 24 etiquetas em cinco famílias, cada uma com o próprio matiz, escolhidas num leque por família com a bolinha da cor na frente. O campo de texto livre saiu, o zod aceita só o que a gama tem, e a ficha mostra a etiqueta na cor da família. Na janela, as ferramentas voltaram para `Badge` de contorno com a bolinha na frente: a lista em colunas durou uma rodada e lia pior.
- **Prévia em `/previa/projetos`** (só em homologação), como a do quadro de tarefas: é onde a tela foi conferida no Chrome sem sessão, com `?projeto=<id>`, `?novo=1` e `?editar=<id>` abrindo a janela e a gaveta. A lista, a janela no computador e no celular, a gaveta e a folha de contato das marcas foram vistas renderizadas.
- **Terceira rodada, a pedido, sobre três prints**: o azulejo da ferramenta passou de círculo a **quadrado de canto squircle** (`ToolTile`), porque as marcas da Adobe são azulejos quadrados e o círculo cortava as quinas e apertava a letra; no squircle encaixam inteiras, e o raio passou a ser de quem monta, concêntrico ao da etiqueta da janela (9, o `md` dela menos fio e recuo), para o azulejo e a etiqueta lerem como uma peça só. As fichas dos seletores da gaveta viraram caixa `md`, pela mesma conta. Conferido na folha de contato das 43, no claro e no escuro, e num recorte ampliado das etiquetas.
- **A janela do projeto no celular ficou como a ficha da tarefa**: uma metade por vez, Informações ou Atividade, com o seletor flutuando acima da bandeja e o arrasto para o lado trocando a metade; os atalhos saíram da linha do nome, porque as ações moram na barra flutuante. O seletor de metade da ficha da tarefa foi **promovido a peça da casa** (`SheetSwitcher`, `components/ui/sheet-switcher/`) em vez de copiado, e a ficha da tarefa passou a usar a peça da casa.

Pendências:

- Contratos e cobranças na ficha do projeto são só o lugar, sem domínio atrás.
- A capa vai embutida na ficha (WebP redimensionado no navegador) porque não há storage de arquivo para projeto ainda; o store é em memória e zera no reinício.
- Visão em tabela e o seletor de visão; gravar de verdade por `service.ts` com Server Action e `api/v1`, com a leitura da ficha ganhando o Route Handler para o aplicativo.
- O leque do cartão ainda só abre, edita e leva ao quadro: pausar, concluir, histórico, cobrança e excluir esperam a regra.
...
- **Dois acertos na janela de vincular, do relato de que sobrava uma faixa branca embaixo do container flutuante.** A folga da barra estava em dois lugares: na lista que rola e no corpo em volta dela. O recuo do corpo não rola com nada, então ele ficava parado como faixa da superfície entre o fim da lista e a base da bandeja (cem pixels, medidos), com a barra pousando em cima dela em vez de encaixar no vão. Agora ela existe uma vez, no fim do que de fato desce, e o corpo devolve a dele por `.body:has(.picker)`; com a lista rolada até o fim, o último registro fecha em 743 e a barra começa em 776. No caminho tentei mover a rolagem para o corpo e o seletor perdeu a busca e as abas presas no topo — a captura de tela mostrou o que a medição não pegava. E o fio da barra antes do X passou a depender de haver algo para separar dele: numa barra de um botão só, ele aparecia solto ao lado do sair.
- **A mesma faixa na subtarefa e no anexo, por outra causa.** Nos vínculos ela vinha de dois recuos somados; nestas duas vem de o conteúdo ser curto e não rolar: a trilha do meio da moldura era `minmax(0, 1fr)`, que estica o corpo até o fim da caixa, e o recuo da barra ficava lá embaixo como superfície pintada, com a barra pousando em cima em vez de encaixar no vão. Em `minmax(0, auto)` a bandeja fecha no último campo (subtarefa caiu de 717 para 556px, anexo para 364) e o recuo vira o vão. O piso zero do `minmax` guarda o caso longo, em que o corpo encolhe e rola. Tentei antes `max-height` no corpo e a barra passou a cobrir o campo de prazo: a captura mostrou, a medição não. Três janelas, uma regra.
- **As duas abas da ficha no celular, a pedido.** A raiz era a ficha vazar pelo pé: o piso de `85dvh` descontava a área segura mas não a **alça de arrastar** do topo, então o miolo pedia dezessete pixels a mais do que a bandeja tem e o compositor terminava fora da janela, cortado, com a barra por cima do campo. Em Informações o recuo de fim ganhou um degrau além do inset, porque o último bloco fechava em 777 contra a barra em 776 — encostava. Em Atividade o cartão do comentário voltou a ser só o campo (148 para 72px de altura): a folga da barra saiu de dentro dele e foi para o compositor em volta, que empurra o cartão para dentro da janela em vez de inchar a caixa e deixar o texto boiando no alto de um retângulo vazio; agora o cartão fecha em 761 e a barra começa em 776. A conversa vazia passou a ficar centrada no que sobra, em vez de colada no topo com meia tela de nada abaixo. Desktop conferido por captura: duas colunas e a fila de ações do compositor intactas.
- **Cinco acertos da rodada de 2026-09-11, a pedido.** **Gravar áudio tem onda**: o `useVoiceRecorder` passou a ler o volume do microfone por `AnalyserNode` e a devolver um `level` de 0 a 1, e a `LiveWave` desenha sete barras que inflam com a fala, as do meio mais que as das pontas, em `scale`, que o compositor resolve sem tocar em layout. O ponto que piscava num intervalo fixo saiu, porque ele não dizia que havia som entrando. A onda e o relógio aparecem **dentro do cartão do comentário**, e não só no botão, porque no celular o microfone mora na barra flutuante e o único sinal era o glifo virar o de encerrar. Com movimento reduzido as barras param numa altura média. **O leque do arroba veste o cartão**: superfície opaca (`--color-bg`), o fio e o raio do campo, sombra média e um título dizendo o que a lista é. O vidro saiu porque a receita das camadas é feita para flutuar sobre a página, e esta caixa flutua sobre o cartão, que já é opaco: vidro sobre opaco não borra nada e só entrega cor lavada. **O quadro ganhou setenta e seis pixels de altura no celular**: a concha reserva `touch-target` mais `space-8` no pé para a barra, e o quadro, que tem altura fixa e já abre a folga por dentro do trilho, contava a reserva duas vezes, então a prancha terminava em 768 numa tela de 844 com a barra flutuando sobre o vazio. A tela passou a medir `100dvh` e a coluna foi de 612 para 688px. **O visualizador de anexo entrou no contrato**: as quatro ações saíram da fila que pousava sobre a imagem e foram para a barra flutuante (abrir em outra aba como principal, baixar e copiar em glifo, o X para fechar), registradas de dentro da `Dialog`; a janela caiu para `md` e o palco desconta a folga da barra. **E a ficha abre sempre em Informações**: a janela é uma só para o quadro inteiro e fica montada entre uma abertura e outra, então quem tivesse ido para a conversa numa tarefa abria a seguinte já na conversa.
