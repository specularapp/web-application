...
- Fios da gaveta e da caixa de troca alinhados ao menu: 0,6px em `--color-border`, no lugar de 1px e de `--color-separator`, que é quase três vezes mais opaco e virava um risco preto numa coluna estreita. O divisor precisa de `&&` para ganhar do módulo do `Separator`, que alveja `.separator[data-orientation]`.- `CommandPalette` saiu de stub e virou a busca do produto, aberta pelo campo do menu, pela barra flutuante do celular e pelo atalho de teclado: campo em cima, atalhos em pílula, três recentes e sugestões, tudo saindo de `nav.ts`, que já tem `href` tipado por rota. `navLinks()` e `navHighlights` novos ali.
...
- Um `listbox` só com o rótulo de grupo como item de apresentação: um por seção duplicaria o id que `aria-controls` aponta. E a janela monta ao abrir em vez de ficar montada com `open={false}`, senão o reset de estado virava escrita de estado em efeito, que o lint barra.- Busca lapidada: as pílulas ganharam matiz próprio pela receita de cor do Badge, canto no raio `md` pelo sistema da casa e uma barra fina de rolagem no lugar da escondida. Os rótulos "Recentes" e "Sugestões" voltaram a alinhar com as opções: eles moram dentro da lista, que já abre o recuo lateral, e o recuo cheio somava duas vezes.- Barra de rolagem: a primeira tentativa não mudou nada na tela, e o motivo é que as duas famílias não convivem. Assim que a largura ou a cor padrão da barra aparecem na regra, o Chrome descarta os pseudo-elementos inteiros e cai na barra nativa, que no Windows vem grossa e com uma seta em cada ponta. Agora as propriedades padrão vivem dentro de `@supports not selector(::-webkit-scrollbar)`, que só o Firefox atende, e o desenho fica nos pseudo-elementos: 2px, sem botão nas pontas.
- Linha da busca enxuta: a seção virou etiqueta na outra ponta e a seta de abrir saiu, porque a janela inteira já é uma lista de coisas que abrem.- Busca de novo: a etiqueta da seção ganhou o matiz da própria seção, declarado na pasta e no grupo em `nav.ts` e propagado por `navLinks()`; o campo virou caixa própria sem a linha embaixo, na forma do gatilho no menu; a lista apertou o recuo; e a barra ficou em 3px.- Busca: o campo voltou a ser a faixa do topo com linha embaixo, a caixa própria foi desfeita a pedido. Entrou linha entre Recentes e Sugestões, no desenho do menu; a linha da lista desceu para a altura do menu (36px, voltando aos 44 em ponteiro grosso); e a régua de teclas ganhou fechar, navegar e abrir.
- O tint do `Badge` ficou mais apagado, de 14% para 9% no claro e de 18% para 12% no escuro, com a participação virando variável (`--badge-tint-share`) para não repetir a fórmula em cada consumidor. Vale para todo badge do app.- As duas áreas que rolam na busca desbotam nas pontas por `mask-image`: sobre vidro, uma faixa de cor por cima viraria mancha em vez de sumiço. A faixa tem o tamanho do próprio recuo, então cai sobre o vão e não sobre o texto quando não há o que rolar. O recuo das pílulas passou a bater com o da linha da lista.- Barra de rolagem virou desenho único em `globals.css`, valendo em tudo que rola, e o mixin `thinScrollbar` saiu junto com os `scrollbar-width: thin` do painel de auth e da tela de plano: declarar a largura padrão desligava o desenho global naquele elemento e devolvia a barra nativa.
- Fade da base da lista maior que o do topo, porque ali ela encosta na régua de teclas, e pílula em 36px no toque: com 44 a faixa do topo comia um terço da bandeja.- Busca com mais respiro: as duas faixas voltaram a recuar 16px, a linha subiu para 40px e a janela passou a usar o mesmo canto `3xl` da gaveta, que é a mesma família e destoava.
- O botão de busca no menu mostra o atalho inteiro, modificador e letra. O rótulo vem de `useCommandKey`, que responde Ctrl no servidor e troca para ⌘ na hidratação em Mac, por `useSyncExternalStore`, então nada é escrito em efeito nem há discordância de marcação.- Régua de teclas com respiro: 12px de recuo vertical no lugar de 8, mais vão entre os grupos e entre a tecla e a palavra. Antes disso, o mesmo pedido foi lido duas vezes como sendo a lista e as pílulas, que acabaram ganhando folga sem precisar.
Pendências:
