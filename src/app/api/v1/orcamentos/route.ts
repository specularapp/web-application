/**
 * Os orçamentos para o aplicativo. Mesma regra da web, pelo mesmo `service.ts`: aqui só muda a casca, que
...
export async function GET(request: Request) {
  ...
  const page = await listQuotes(auth.session.supabase, auth.session.organizationId, parseQuotesQuery(Object.fromEntries(params)));
  return Response.json(page);
}

export async function POST(request: Request) {
  const auth = await authorizeDomain(request, "quotes-write");
  if ("response" in auth) return auth.response;
