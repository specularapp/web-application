import { z } from "zod";
import { partyOf } from "./labels";
import { GRID_PER_PAGE_DEFAULT, MAX_PER_PAGE, TABLE_PER_PAGE, defaultQuery, directionFilterValues, methodFilterValues, statusFilterValues, type ChargesListPage, type ChargesQuery } from "./list-options";
import { chargeOpen, chargeStatusOf, nextInstallment, todayIso, type Charge, type ChargeDirection, type ChargeStatus } from "./summary";
...
const querySchema = z.object({
  ...
  direction: z.enum(directionFilterValues).catch(defaultQuery.direction),
...
export function parseChargesQuery(params: Record<string, string | undefined>, fallbackPageSize = TABLE_PER_PAGE): ChargesQuery {
  const { pageSize, ...rest } = querySchema.parse({ search: params.busca, direction: params.lado, status: params.situacao, method: params.forma, page: params.pagina, pageSize: params.porPagina });
...
export function listCharges(items: Charge[], query: ChargesQuery, today = todayIso()): ChargesListPage {
  ...
  const sides: Record<ChargeDirection, number> = { incoming: 0, outgoing: 0 };
  ...
  return { items: filtered.slice(start, start + query.pageSize), total: filtered.length, counts, sides, totals };
}
