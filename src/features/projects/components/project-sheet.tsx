...
/**
 * A ficha do projeto: a capa, quem contratou, o nome com a situação, a descrição, os números em azulejo e as
 * seções nos primitivos de perfil da casa. É o corpo da janela do projeto, extraído para uma peça só
 * (2026-09-20, a pedido de a prévia do editor refletir a visualização completa): antes a coluna da direita do
 * editor mostrava o cartão da grade e uma lista solta de oito fatos, que não era a cara do projeto em lugar
 * nenhum do produto. Agora as duas pontas desenham o mesmo, e um acerto na ficha chega nas duas.
 *
 * O que depende só do projeto desenha na hora; o que vem do servidor (tarefas, orçamentos, equipe) espera a
 * ficha completa. Na prévia não há servidor a esperar, porque o projeto ainda não existe, então essas seções
 * simplesmente não entram, em vez de ficarem num carregando que nunca termina.
 *
 * O estilo mora em `project-dialog.module.css`, junto de quem monta a janela: a ficha é o corpo dela, e
 * separar a folha de estilo em duas só para ter dois arquivos deixaria as medidas da capa e da identidade
 * longe do resto da janela que as emoldura.
 */
export type ProjectSheetProps = {
  project: Project;
  /** A ficha completa vinda do servidor; nula enquanto ela não chega. */
  details?: ProjectDetails | null;
  /** Abre a ficha para editar. Ausente na prévia, e aí o atalho sai. */
  onEdit?: () => void;
  /**
   * A prévia do editor: o projeto ainda não existe, então não há endereço para onde apontar nem ficha
   * completa para esperar. Os atalhos saem e os vínculos ficam para quando ele nascer.
   */
  preview?: boolean;
};
...
export function ProjectSheet({ project, details = null, onEdit, preview = false }: ProjectSheetProps) {
  ...
  return (
    <section className={styles.main} aria-label="Ficha do projeto">
      ...
      <div className={styles.identity}>
        ...
        {/* Na prévia os atalhos saem: o site ainda não foi salvo e o quadro não existe, então eles levariam
      ...
      {/* Os números do projeto em azulejos com fio, o rótulo em cima e o valor embaixo, como na ficha do
          cliente, e a barra do andamento logo abaixo. O que o servidor conta entra quando chega; na prévia
          o lugar dele é ocupado pelas datas, que é o que já se sabe da ficha aberta. */}
      <dl className={styles.tiles}>
        <Tile label="Andamento" value={`${project.progress}%`} />
        {details ? (
          <>
            <Tile label="Tarefas abertas" value={`${details.openTasks} de ${details.totalTasks}`} />
            <Tile label="Orçamentos" value={String(details.quotes.length)} />
          </>
        ) : (
          <>
            <Tile label="Começo" value={shortDate(project.startedAt)} />
            <Tile label="Entrega" value={project.dueAt ? shortDate(project.dueAt) : "Sem prazo"} />
          </>
        )}
      ...
      <div className={styles.sections}>
        <ProfileSection title="Detalhes">
          <ProfileFacts>
            ...
            <ProfileFact icon={IdentificationCardIcon} label="Identificador">
              <Text as="span" variant="subheadline" weight="medium">
                {preview ? "Ao salvar" : project.reference}
              </Text>
            </ProfileFact>
          </ProfileFacts>
        </ProfileSection>
        ...
        {/* O que o projeto liga. Na prévia isso ainda não existe, e dizer isso é melhor que sumir com as
            seções: a pessoa vê onde tarefa, orçamento, contrato e cobrança vão aparecer depois. */}
        {details ? (
          <>
            ...
            {/* Contrato e cobrança são os dois lados que ainda não têm vínculo: as seções já existem, no
                lugar em que ele vai aparecer, para a ficha dizer o que um projeto liga. */}
          ...
          <ProfileSection title="Vínculos">
            <Text variant="footnote" tone="secondary">
              Tarefas, orçamentos, contratos e cobranças aparecem aqui depois que o projeto for criado.
            </Text>
          </ProfileSection>
        ) : (
