...
import styles from "./plan-chooser.module.css";

export type PlanChooserProps = {
  organizationId: string;
  billing: BillingState;
  /** Na prévia os campos de cartão montam, mas nada é cobrado. */
  demo?: boolean;
  /** Acabou: a assinatura entrou, ou a pessoa seguiu com o plano que já tinha. */
  onDone: () => void;
};
...
/**
 * A escolha de plano da casa: **uma só**, do jeito que ela nasceu no passo 3 dos primeiros passos.
 *
 * Logo, título, o alternador de ciclo, os três cartões com o preço que corre ao trocar de ciclo, os
 * recursos de cada um e, quando o plano é pago e não há cartão guardado, o formulário de pagamento no lugar
 * da escolha. No celular a fileira vira carrossel com arraste e alinhamento próprios.
 *
 * Era a etapa do onboarding e virou peça quando o modal central de plano precisou da mesma escolha: toda
 * ação bloqueada da aplicação abre este mesmo mapa de planos, **igual**, sem uma palavra trocada. Um segundo
 * desenho para a mesma escolha seria a segunda tabela de preço da casa, e a que ficaria para trás no dia em
 * que um plano mudar.
 */
export function PlanChooser({ organizationId, billing, demo = false, onDone }: PlanChooserProps) {
    ...
    return (
      <div className={styles.step}>
        ...
        <CheckoutPanel
          ...
          onDone={demo ? () => setCheckout(null) : onDone}
        />
      </div>
    );
  }
