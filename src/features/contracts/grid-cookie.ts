 ...
 * Separado da leitura no servidor pelo mesmo motivo do catálogo e dos projetos: a prancha é componente de
 * cliente e só precisa gravar, e a leitura com zod mora em `list.ts`.
 */
export const CONTRACTS_GRID_COOKIE = "sp-contratos-grade";

/** Grava a medida no cookie, no cliente. Vive fora do componente porque escrever em `document` de dentro dele o lint barra. */
export function saveContractsGridSize(size: number) {
  document.cookie = cookieString(CONTRACTS_GRID_COOKIE, String(size));
}
